...
#   indexing only via labels (label-based indexing)
#     1.1 Series:
...
#   indexing only via integer position (positional indexing)
#     2.1 Series:
...
#
#   3. Selection using []
#   combines both label based and integer position
#     3.1 Series:
...
# Wil raise a KeyError when items not found
# .loc accepts slices of index labels, can use with single labels sequences.
# Does NOT understand positional indexing
# ----------------------------------------------------------------------------
...
# Copy the series
ser2 = ser.copy()  # Because don't want to make any destructive edits
print(ser2)
...
# df.loc[row indexer, col indexer]. col indexer can be omitted to include all columns. row indexer is required
...
# If we have an index out of slicing, will not give error - but index until the last value
...
# If put any list type in columns, pandas will change type to df instead of series, even if the series has 1 value
x = df.loc['2020-01-06':, 'Bday':]
print(x)

print(type(df.loc['2020-01-06':, 'Bday':])) # --> <class 'pandas.core.frame.DataFrame'>

x = df.loc['2020-01-06', 'Close':] # single row label AND open column slice = series
print(x)
print(type(x))
...
# Select values for indexes [b, c, d, e]
# import pandas as pd
dfz = pd.DataFrame(
  {
      'col1':range(10),
...
# Select rows b d f j in col1, make sure return data as dataframe

y = dfz.loc[['b', 'd', 'f', 'j'], 'col1':'col1']
# OR
y = dfz.loc[['b', 'd', 'f', 'j'], ['col1']]
print(type(x))

# ----------------------------------------------------------------------------
#  2. Selection using .iloc
#   (only works with positions)
# index based location - positional based indexing
# ----------------------------------------------------------------------------
...
# will not raise error if out of bounds in slice
x = ser.iloc[1:100] # no error
...
s2.iloc[0] = int(0)
print(s2)
# Automatically changed to float due to forcing homogenous data
...
x  = df.iloc[1:1000, :] # will produce an intersection until last value even though index is oob
print(x)

# x--> empty DF
x  = df.iloc[100:101] # empty df
print(x)
...
# Select values for even-indexed rows from col2
dfa = pd.DataFrame(
  {
      'col1':range(0, 20),
      'col2':range(20, 40),
...
# OR
rows = list(range(0, len(dfa), 2))
result = dfa.iloc[rows, [1]]
print(result)
