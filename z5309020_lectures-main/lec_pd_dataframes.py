...
# Just like any series, you can access the index using: Index is the same for each column, no difference in
# specifying which column to call the index on
print(col0.index)
print(df.index)
...
sorted_ser = new_ser.sort_index() # Only modifies copy, not original
print(new_ser)
new_ser.sort_index(inplace = True) # This modifies original
print(new_ser)
print(sorted_ser)
...
# In class exercise: Construct this data frame from scratch
import pandas as pd
dtfrm = pd.DataFrame(
            {
                'a' : [1, 2, 3],
